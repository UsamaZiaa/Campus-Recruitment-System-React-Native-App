...
export default function StudentSignup({navigation}) {
  ...
  const uploadPic = async (props) => {
    let imgURI = image
    // console.log('Complete Data',imgURI, props)
    ...
    Firebase.database().ref('/').child('students').child(user.uid).set(user);
    setLoading(false);
    Alert.alert('Congratulations','Account Created Successfully', [
      {text: 'Finish', onPress: () => navigation.navigate('Student')}
    ])
}
        ...
        null
          ...
          {(props)=>(
            <View style={styles.FormContainer}>
              ...
              />
              <Text style={styles.errorText}>{props.touched.skills && props.errors.skills}</Text>  

...
export default function ViewCompany({navigation}) {
  const [companies, setCompanies] = useState([])

  useEffect(() => {
    const companiesData =  Firebase.database().ref('companies');
    companiesData.on('value',data => {
  ...
  const deleteUser = (uid) => {
    console.log(uid)
    Firebase.database().ref('companies').child(uid).remove()
}

  return (
      <View style={styles.container}>
        <Header title="View Companies" Logout = {Logout} />
        <FlatList
            ...
            renderItem={({item})=>(
                ...
                activeOpacity={.6}
                style={styles.card}
                onPress = {() => navigation.navigate('CompanyProfile', item)}
                >
